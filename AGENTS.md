...
- Если ESP8266 не получает IP домашней Wi-Fi сети за 30 секунд, он поднимает fallback AP `AiDvor-ESP-<chipId>`; локальные страницы доступны через `http://192.168.4.1`.
...
8. Перед любыми изменениями кода, конфигов, документации, прошивок или команд, влияющих на проект/устройства, сначала обсудить задачу.
9. До начала разработки агент кратко излагает план изменений и ждет явное подтверждение пользователя.
10. Исключение: информационные ответы без изменения файлов, конфигов, устройств и окружения.
