<x-layouts.admin title="Telegram-заявки | Market Admin">
    <h1 class="mb-6 text-2xl font-semibold">Telegram-заявки</h1>
        ...
        <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-200">
                @forelse ($conversations as $conversation)
                    <tr>
                        <td class="px-4 py-3">
                            <div class="font-medium">{{ $conversation->telegram_username ? '@'.$conversation->telegram_username : $conversation->telegram_user_id }}</div>
                        ...
                        <td class="px-4 py-3 text-right"><a class="text-emerald-700" href="{{ route('admin.telegram.show', $conversation) }}">Открыть</a></td>
                    </tr>
                @empty
                    <tr>
                        <td class="px-4 py-6 text-slate-600" colspan="6">Telegram-заявок пока нет.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
