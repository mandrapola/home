/*
  AiDvor station Wi-Fi proxy v1, ESP8266 side.
...
const size_t JSON_MAX = 900;
const size_t RESPONSE_MAX = 320;
const size_t RELAY_COUNT = 8;
...
char relayState[RELAY_COUNT] = { '0' };
char tankLevel[16] = "";
char lightLevelRaw[16] = "";
char airHumidity[16] = "";
char airTemperature[16] = "";
char atmPressure[16] = "";
char waterTemperature[16] = "";
...
static void updateRelayStateFromKey(const char *key, const char *value) {
  ...
  if (relayIndex < 0 || relayIndex >= (int) RELAY_COUNT) {
    return;
  }
...
static void updateStateFromCsv(const char *csv) {
  ...
  while (readCsvToken(&cursor, key, sizeof(key), value, sizeof(value))) {
    ...
    if (strcmp(key, "tank_level") == 0) {
      rememberValue(tankLevel, sizeof(tankLevel), value);
    ...
    } else if (strcmp(key, "atm_pressure") == 0) {
      rememberValue(atmPressure, sizeof(atmPressure), value);
    } else if (strcmp(key, "water_temperature") == 0) {
      rememberValue(waterTemperature, sizeof(waterTemperature), value);
    }
  }
}
...
static void sendAllRelaysOffToUno() {
  ...
  snprintf(
    requestLine,
    sizeof(requestLine),
    "relay_1=0;relay_2=0;relay_3=0;relay_4=0;relay_5=0;relay_6=0;relay_7=0;relay_8=0;monitor=E%03d",
    status
  );
...
static void sendLocalPage() {
  ...
  html += F("const labels={tank_level:'Уровень бака',light_level_raw:'Освещенность',air_humidity:'Влажность воздуха',air_temperature:'Температура воздуха',atm_pressure:'Атм. давление',water_temperature:'Температура воды'};");
...
static void handleRelay() {
  ...
  if (relayIndex < 0 || relayIndex >= (int) RELAY_COUNT || (value != "0" && value != "1")) {
  ...
  snprintf(
    requestLine,
    sizeof(requestLine),
    "relay_1=%c;relay_2=%c;relay_3=%c;relay_4=%c;relay_5=%c;relay_6=%c;relay_7=%c;relay_8=%c;monitor=local",
    relayState[0],
    relayState[1],
    relayState[2],
    relayState[3],
