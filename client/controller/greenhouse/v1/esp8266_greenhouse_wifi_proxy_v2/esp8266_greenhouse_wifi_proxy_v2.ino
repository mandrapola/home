...
const char DEFAULT_SETUP_AP_PASSWORD[] = "aidvor1234";
...
static const char *shortWifiModeName() {
  if (fallbackApActive) {
    return "AP";
  }

  return WiFi.status() == WL_CONNECTED ? "STA" : "OFF";
}

static const char *shortErrorCode() {
  if (strcmp(lastError, "none") == 0) return "NONE";
  if (strstr(lastError, "wifi") != NULL) return "WIFI";
  if (strstr(lastError, "tls") != NULL) return "TLS";
  if (strstr(lastError, "token") != NULL) return "TOK";
  if (strstr(lastError, "config") != NULL) return "CFG";
  if (strstr(lastError, "csv") != NULL) return "CSV";
  if (strstr(lastError, "post") != NULL) return "POST";
  if (strstr(lastError, "http") != NULL) return "HTTP";
  if (strstr(lastError, "cloud") != NULL) return "CLD";
  return "ERR";
}
...
static const char *fallbackApPassword() {
  ...
  return DEFAULT_SETUP_AP_PASSWORD;
}
